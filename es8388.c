...
static int es8388_set_gpio(int gpio, bool level)
{
	...
	if (!es8388)
		return 0;
    if ((gpio & ES8388_CODEC_SET_HP) && es8388
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	if (gpio_get_value(es8388->hp_det_gpio)){
        ...
        snd_soc_component_write(es8388->component, 0x0b, 0x82);
    }
	else {
		es8388->hp_inserted = 1;
        snd_soc_component_write(es8388->component, 0x0b, 0x02);
    }
...
static unsigned int es8388_read_reg_cache(struct snd_soc_component *codec,
					  unsigned int reg)
{
			...
			unsigned int value)
{
	...
	ret = i2c_master_send(es8388->i2c, data, 2);
	if (ret == 2)
		return 0;
	if (ret < 0)
		return ret;
...
static int es8388_resume(struct snd_soc_component *codec)
{
	...
	snd_soc_component_write(codec, ES8388_DACPOWER, 0x0c);
	snd_soc_component_write(codec, ES8388_ADCPOWER, 0x00);
...
static int es8388_probe(struct snd_soc_component *codec)
{
	...
	snd_soc_component_write(codec, 0x19, 0x02);
	/* pdn_ana=0,ibiasgen_pdn=0 */
	snd_soc_component_write(codec, 0x04, 0x0c);
	usleep_range(18000, 20000);
	snd_soc_component_write(codec, 0x2e, 0x00);
	snd_soc_component_write(codec, 0x2f, 0x00);
	snd_soc_component_write(codec, 0x30, 0x08);
	snd_soc_component_write(codec, 0x31, 0x08);
	usleep_range(18000, 20000);
	snd_soc_component_write(codec, 0x30, 0x0f);
	snd_soc_component_write(codec, 0x31, 0x0f);
	usleep_range(18000, 20000);
	snd_soc_component_write(codec, 0x30, 0x18);
	snd_soc_component_write(codec, 0x31, 0x18);
	usleep_range(18000, 20000);
	/* pdn_ana=0,ibiasgen_pdn=0 */
	snd_soc_component_write(codec, 0x04, 0x2c);

	snd_soc_component_write(codec, ES8388_DACCONTROL3, 0x06);
	es8388_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
    printk("------------------es8388_probe_end-----------------\n");

	return 0;
}

static void es8388_remove(struct snd_soc_component *codec)
{
	es8388_set_bias_level(codec, SND_SOC_BIAS_OFF);
	return ;
}

static struct snd_soc_component_driver soc_component_dev_es8388  = {
	.probe = es8388_probe,
			    ...
			    const struct i2c_device_id *id)
{
    ...
    es8388->hp_ctl_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "hp-ctl-gpio", 0, &flags);
	if (es8388->hp_ctl_gpio < 0) {
		...
		ret = devm_gpio_request_one(&i2c->dev, es8388->hp_ctl_gpio, GPIOF_DIR_OUT, "hp_ctl_gpio");
		if (ret != 0) {
    ...
    printk("------------------i2c_probe_end-----------------\n");

	return devm_snd_soc_register_component(&i2c->dev,
				      &soc_component_dev_es8388,
				      &es8388_dai, 1);
...
void es8388_i2c_shutdown(struct i2c_client *client)
{
	...
	es8388_set_gpio(ES8388_CODEC_SET_SPK, !es8388->spk_gpio_level);
	mdelay(20);
	snd_soc_component_write(es8388->component, ES8388_CONTROL2, 0x58);
	snd_soc_component_write(es8388->component, ES8388_CONTROL1, 0x32);
	snd_soc_component_write(es8388->component, ES8388_CHIPPOWER, 0xf3);
	snd_soc_component_write(es8388->component, ES8388_DACPOWER, 0xc0);
	mdelay(50);
	snd_soc_component_write(es8388->component, ES8388_DACCONTROL26, 0x00);
	snd_soc_component_write(es8388->component, ES8388_DACCONTROL27, 0x00);
	mdelay(50);
	snd_soc_component_write(es8388->component, ES8388_CONTROL1, 0x30);
	snd_soc_component_write(es8388->component, ES8388_CONTROL1, 0x34);
}
