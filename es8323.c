...
//static void hp_det_work_func(struct work_struct *work);
//static DECLARE_DELAYED_WORK(hp_det_work, hp_det_work_func);
...
static void hp_det_work_func(struct work_struct *work)
{
    ...
    for (i = 0; i < 53; i++) {
        reg = i & 0x00ff;
        data = snd_soc_component_read(es8323->component, reg);
        printk("register%x:%x  \n", reg, data);
       // i2c_master_send(es8323->i2c, &reg, 1);
       // i2c_master_recv(es8323->i2c, &data, 1);
       // printk("register%d: %x \n ", reg, data);
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	*/
	...
	//schedule_delayed_work(&hp_det_work, msecs_to_jiffies(500));
...
static void es8323_set_linein(unsigned int flags)
{
	...
	if (flags) {
		...
		snd_soc_component_write(es8323->component, 0x26, 0x00);
		snd_soc_component_write(es8323->component, 0x27, 0x40);
		snd_soc_component_write(es8323->component, 0x2a, 0x40);
		snd_soc_component_write(es8323->component, 0x0b, 0x02);
		snd_soc_component_write(es8323->component, 0x04, 0x0c);

		spk_detect_do_switch(1);
	} else {
		printk("ES8323 exit line in mode\n");

		snd_soc_component_write(es8323->component, 0x26, 0x12);
		snd_soc_component_write(es8323->component, 0x27, 0xb8);
		snd_soc_component_write(es8323->component, 0x2a, 0xb8);
		snd_soc_component_write(es8323->component, 0x0b, 0x82);
		snd_soc_component_write(es8323->component, 0x04, 0x3c);
...
static int es8323_write(struct snd_soc_component *component, unsigned int reg,
			unsigned int value)
{
 ...
 */
static int es8323_set_dai_sysclk(struct snd_soc_dai *component_dai,
...
static int es8323_set_dai_fmt(struct snd_soc_dai *component_dai, unsigned int fmt)
{
	struct snd_soc_component *component = component_dai->component;
	u8 iface = 0;
	u8 adciface = 0;
...
{
	if ( !(int)flags )
		snd_soc_component_write(es8323_private->component, 0x0b, 0x82);
	else
        snd_soc_component_write(es8323_private->component, 0x0b, 0x02);
...
static int es8323_probe(struct snd_soc_component *component)
{
	...
	if (es8323->hp_det_gpio != INVALID_GPIO) {
       // schedule_delayed_work(&hp_det_work, msecs_to_jiffies(1000));
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,0,0)
    static int es8323_remove(struct snd_soc_component *component)
    {
...
static struct snd_soc_component_driver soc_component_dev_es8323 = {
	.probe = es8323_probe,
...
void es8323_i2c_shutdown(struct i2c_client *client)
{
	...
	es8323_set_gpio(ES8323_CODEC_SET_HP, !es8323->hp_gpio_level);
	mdelay(20);
	snd_soc_component_write(es8323->component, ES8323_CONTROL2, 0x58);
	snd_soc_component_write(es8323->component, ES8323_CONTROL1, 0x32);
	snd_soc_component_write(es8323->component, ES8323_CHIPPOWER, 0xf3);
	snd_soc_component_write(es8323->component, ES8323_DACPOWER, 0xc0);
	mdelay(50);
	snd_soc_component_write(es8323->component, ES8323_DACCONTROL26, 0x00);
	snd_soc_component_write(es8323->component, ES8323_DACCONTROL27, 0x00);
	mdelay(50);
	snd_soc_component_write(es8323->component, ES8323_CONTROL1, 0x30);
	snd_soc_component_write(es8323->component, ES8323_CONTROL1, 0x34);
}
